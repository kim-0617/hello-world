...
import pygame
from pygame.constants import K_SPACE, SRCCOLORKEY
import os
##################################################################
# 기본 초기화 (반드시 해야 하는 것들)
...
while runnig:
    ...
    if event.type == pygame.KEYDOWN:
        if event.key == pygame.K_LEFT:
            ...
            character_to_x += character_speed
        elif event.key == pygame.K_UP: # 무기 발사
    ...
    elapsped_time = (pygame.time.get_ticks() - start_ticks) / 1000
    timer = game_font.render(f"Time {int(total_time - elapsped_time)}",True,(255,255,255))
    screen.blit(timer, (10,10))

    # 시간 초과 했다면
    if total_time - elapsped_time <= 0:
